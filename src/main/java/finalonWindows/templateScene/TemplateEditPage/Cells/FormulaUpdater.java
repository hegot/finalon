            ...
            public void handle(ActionEvent e) {
                try {
                    for (Map.Entry<Integer, AutoCompleteTextArea> entry : formulaMap.entrySet()) {
                        FormulaStorage.addFormulaDelete(entry.getKey());
                    }
                    table.getItems().remove(selectedItem);
                    TemplateEditTable.getItems().remove(selectedItem);
                    dialog.close();
                ...
                try {
                    ...
                    for (Map.Entry<Integer, AutoCompleteTextArea> entry : formulaMap.entrySet()) {
                        ...
                        if (contains) {
                            ...
                            Formula formulaUpdated = FormulaStorage.getFormulaUpdate(formula.getId());
                            if (formulaUpdated != null) {
                                formula = formulaUpdated;
                            }
                            formula.setValue(output);
                            FormulaStorage.addFormulaUpdate(formula);
                        }
                    }
                    if (!contains) {
                        table.getItems().remove(selectedItem);
